...
Publisher.prototype.publish = function (headers, options) {
  ...
  return through.obj(function(file, enc, cb) {
      ...
      if (!file.isBuffer()) {
        this.emit('error',
          new gutil.PluginError(PLUGIN_NAME, 'Only Buffer content is supported'));
        return cb();
      }
      ...
      if (!fileList) {
        _this.client.listObjects({
          ...
          if(res.IsTruncated) {
            this.emit('error',
              new gutil.PluginError(PLUGIN_NAME, 'Bucket contains more than 1000 objects'));            
          }
          fileList = keyMap(res.Contents);
          var key = file.path.replace(/^\//,"");
          update.call(_this, file, etag, fileList[key], options, cb)
        })
      } else {
        update.call(_this, file, etag, fileList[file.path], options, cb)
      }
...
function update(file, currentETag, S3Info, options, cb) {

  // skip: no updates allowed
  var noUpdate = options.createOnly && S3Info && S3Info.ETag;

  // skip: file are identical
  var noChange = !options.force && S3Info && S3Info.ETag === currentETag && S3Info.Size > 0; // with ETags we cannot distinguish between zero-content files

  if (noUpdate || noChange) {
    file.s3.state = 'skip';
    file.s3.etag = S3Info.ETag;
    file.s3.date = new Date(S3Info.LastModified);
    return cb(null, file);

    // update: file are different
  } else {
    file.s3.state = S3Info ? 'update' : 'create';

    this.client.putObject(toAwsParams(file), function(err) {
      if (err) return cb(err);
